...
UA="Mozilla/5.0 CryptalystsFormDiscovery/1.0"

CONTACT_PATHS=["","/contact","/contact-us","/contacts","/support","/help","/about","/team","/links"]
FORM_PLATFORMS=["typeform.com","tally.so","forms.gle","docs.google.com/forms","airtable.com","formspree.io","hubspot","hsforms","jotform.com","notion.site","paperform.co"]
...
def discover(item):
    ...
    for page in candidate_pages(item["website"]):
        ...
        if form:
            return wp_update(post_id,form_url=form,form_type=typ,status="FOUND",notes=f"Form found on {final}")
    ...
    for u in list(dict.fromkeys(found))[:6]:
        ...
        form,typ=detect_form(html,final)
        if form:
            return wp_update(post_id,form_url=form,form_type=typ,status="FOUND",notes=f"Form found on linked page {final}")
...
def main():
    ...
    for it in items:
        ...
        discover(it)
    print(f"Done. Scanned {len(items)}.")
